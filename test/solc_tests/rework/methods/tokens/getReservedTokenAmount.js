        ...
        describe("view currentReservedTokenAmount(address)", async function () {
            ...
            describe("participant has no contributions", async function () {
                ...
                it("Returns 0 at any stage", async function () {
                    ...
                    const ContractContributionTokens = ParticipantsTotalStats.totalReservedTokens;

                    this.jsValidator.setBlockNumber(currentBlock);
                    let validatorTokenAmount = this.jsValidator.currentReservedTokenAmount(participantAddress);

                    let currentReservedTokenAmount = await this.ReversibleICO.methods.currentReservedTokenAmount(participantAddress).call();
                    expect(currentReservedTokenAmount).to.be.equal(ContractContributionTokens);
                    expect(currentReservedTokenAmount.toString()).to.be.equal(validatorTokenAmount.toString());
                    ...
                    validatorTokenAmount = this.jsValidator.currentReservedTokenAmount(participantAddress);
                    currentReservedTokenAmount = await this.ReversibleICO.methods.currentReservedTokenAmount(participantAddress).call();
                    expect(currentReservedTokenAmount.toString()).to.be.equal(validatorTokenAmount.toString());
                    expect(currentReservedTokenAmount.toString()).to.be.equal("0");
            ...
            describe("participant has made a contribution", async function () {
                ...
                it("Returns participant's purchased token amount before stage 1 start_block", async function () {
                    ...
                    const ContractContributionTokens = ParticipantTotalStats.totalReservedTokens;

                    const currentReservedTokenAmount = await this.ReversibleICO.methods.currentReservedTokenAmount(participantAddress).call();
                ...
                it("Returns proper amount at stage 1 start_block", async function () {
                    ...
                    const currentReservedTokenAmount = await this.ReversibleICO.methods.currentReservedTokenAmount(participantAddress).call();
                    const validatorTokenAmount = this.jsValidator.currentReservedTokenAmount(participantAddress);
                ...
                it("Returns 0 locked tokens at stage 12 end_block ( also known as BuyPhaseEndBlock )", async function () {
                    ...
                    let ContractContributionTokens = ParticipantsTotalStats.totalReservedTokens;
                    expect(parseInt(ContractContributionTokens)).to.be.above(0);
                ...
                it("Returns 0 locked tokens after BuyPhaseEndBlock", async function () {
                    ...
                    ContractContributionTokens = ParticipantsTotalStats.totalReservedTokens;
                    expect(parseInt(ContractContributionTokens)).to.be.above(0);
